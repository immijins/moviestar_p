...
import com.example.moviestar.repository.ReplyRepository;
...
public class ReplyServiceImpl implements ReplyService {
    ...
    public void modify(ReplyDTO replyDTO) {
        ...
        Reply reply = replyOptional.orElseThrow();
        reply.changeText(replyDTO.getReplyText(), replyDTO.getMovieLike()); // 댓글 내용만 수정 가능
    ...
    @Override
    public PageResponseDTO<ReplyDTO> getListOfMovie(Long no, PageRequestDTO pageRequestDTO) {
        Pageable pageable = PageRequest.of(pageRequestDTO.getPage() <= 0 ? 0 : pageRequestDTO.getPage() - 1,
                pageRequestDTO.getSize(), Sort.by("rno").ascending());
        Page<Reply> result = replyRepository.listOfMovie(no, pageable);
